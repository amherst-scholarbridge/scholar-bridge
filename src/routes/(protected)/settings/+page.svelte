	...
	{#if $currentUser}
		<section class="card p-4 w-full text-token">
			...
			<p class="mt-2">
				...
				<span class="font-bold">"{$currentUser.is_tutee ? "tutee" : "tutor"}"</span>.
			</p>
			{#if !$currentUser?.is_tutee}
			<p class="italic mb-2">
				Note: If you are NOT a tutor (ie: you are here to be tutored) but your account type
			...
			{#if $currentUser?.is_tutee}
			<p class="italic mb-2">
				Note: If you ARE a tutor but your account type
