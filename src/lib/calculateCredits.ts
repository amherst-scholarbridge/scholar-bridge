...
export function calculateRequiredCredits(user: any, type: RecievedCredit["type"]): number {
	if (user.is_tutee) {
		throw new Error("Cannot calculate required credits for a user who is not of proper age.");
	}
	...
	if (user.graduationYear == 2026) {
		...
		return "Error : Cannot calculate credits for a tutor who isn't a current sophomore/junior/senior" as any as number;
